-- Fix subcategories structure to match categories pattern
-- Drop the old subcategories table and recreate with proper multilingual structure

-- First, drop the existing table (this will cascade delete all data)
DROP TABLE IF EXISTS public.subcategories CASCADE;

-- Create new subcategories table with JSONB content like categories
CREATE TABLE public.subcategories (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  category_id UUID REFERENCES public.categories(id) ON DELETE CASCADE NOT NULL,
  slug TEXT NOT NULL,
  title JSONB NOT NULL, -- {"en": "English Title", "tr": "Turkish Title"}
  body_text JSONB, -- {"en": "English text", "tr": "Turkish text"}
...
COMMENT ON TABLE public.subcategories IS 'Subcategories for main categories with multilingual JSONB content';
COMMENT ON COLUMN public.subcategories.title IS 'Subcategory title in JSONB format {"en": "English", "tr": "Turkish"}';
COMMENT ON COLUMN public.subcategories.body_text IS 'Detailed description text in JSONB format {"en": "English", "tr": "Turkish"}';
